...
@dataclass
class Movie:
    """A single movie or show entry as exposed by a streaming provider."""
    ...
    language: str = "en"
    user_rating: float = 0.0  # 0.0 - 10.0
...
class ViewingHistoryEntry:
    """A single watch event recorded from a user's streaming account."""

    title_id: str
    title: str
    platform: str
    watched_at: str  # ISO 8601 timestamp
...
@dataclass
class Feedback:
    """Explicit feedback the user gives on a recommendation."""

    title_key: str
    signal: str  # "liked", "disliked", "watched", "not_interested"
    note: str = ""
    created_at: str = ""
...
@dataclass
class UserProfile:
    """Lightweight description of the user's stated preferences."""
...
class Recommendation:
    """A ranked recommendation returned by the agent."""
