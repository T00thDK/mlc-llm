...
Thread-safe via RLock. Stores profile, viewing history, feedback events,
and cached preference weights in a single JSON file.
"""
...
class MemoryStore:
    ...
    def add_history(self, entries: List[ViewingHistoryEntry]) -> None:
        ...
        with self._lock:
            existing = {
                (e["title_id"], e["platform"], e["watched_at"])
    ...
    def _load(self) -> None:
        ...
        self._state.setdefault("profile", None)
        self._state.setdefault("history", [])
        self._state.setdefault("feedback", [])
        self._state.setdefault("preference_weights", {})
